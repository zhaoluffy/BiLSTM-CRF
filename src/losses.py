...
def cross_entropy_sequence_loss(logits, targets, sequence_length):
    ...
    with tf.name_scope("cross_entropy_sequence_loss"):
        ...
        loss_mask = tf.sequence_mask(tf.to_int32(sequence_length))
        losses = losses * tf.to_float(loss_mask)
        loss = tf.reduce_mean(losses)
        return loss


def crf_loss(logits, targets, sequence_length):
    with tf.variable_scope("crf_loss"):
        log_likelihood, transition_param = tf.contrib.crf.crf_log_likelihood(logits,
                                                                             targets,
                                                                             sequence_length)
